import { cborgJson, CID, base64 } from '../../deps.ts'
import { Token, Type } from '../cborg/lib.ts'
import type { ByteView } from '../multiformats/types.ts'
import type { ToString } from '../multiformats/link.ts'
import type { TagDecoder } from '../cborg/interface.ts'
...
class DagJsonTokenizer extends cborgJson.Tokenizer {
  ...
  constructor(data: Uint8Array, options: Record<string, unknown>) {
    super(data, options)
    this.tokenBuffer = []
  }
 ...
 * @returns {ByteView<T>}
 */
export const encode = <T>(node: T): ByteView<T> => cborgJson.encode(node, encodeOptions)
 ...
 * @returns {T}
 */
export const decode = <T>(data: ByteView<T>): T => {
 ...
 * @returns {T}
 */
export const parse = <T>(data: ToString<T>): T => decode(utf8Encoder.encode(data))
